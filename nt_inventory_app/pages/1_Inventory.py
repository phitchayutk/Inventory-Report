...
with st.container():
    ...
    inv_file = st.file_uploader("Upload archive (.zip / .7z)", type=['zip','7z'], key="inv_upload")
    if inv_file:
...
if st.button("🔍 Process Inventory", type="primary", use_container_width=True):
    ...
    st.success(f"✅ **{len(all_rows):,} records** จาก **{n:,} ไฟล์** (show inventory: {normal_n:,} | admin: {admin_n:,})")
    st.rerun()
