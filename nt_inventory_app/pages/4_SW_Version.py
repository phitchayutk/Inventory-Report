...
st.info("ℹ️ Process หน้านี้ก่อนหน้า Inventory — SW Version จะถูก merge เข้า Inventory อัตโนมัติ")
...
if st.button("🔍 Process SW Version", type="primary", use_container_width=True):
    ...
    # Update existing inventory_rows
    inv_rows = st.session_state.get('inventory_rows', [])
    updated = sum(1 for r in inv_rows if r.get('Hostname','') in version_map)
    for r in inv_rows:
        hn = r.get('Hostname','')
    ...
    st.success(
        f"✅ **{len(version_map):,} devices** จาก **{n:,} ไฟล์**"
        + (f" | อัปเดต Inventory {updated:,} rows" if updated else "")
    )
    st.rerun()
...
if st.session_state.version_map:
    ...
    tab1, tab2 = st.tabs(["📋 Hostname → Version", "📊 Version Summary"])
    with tab1:
