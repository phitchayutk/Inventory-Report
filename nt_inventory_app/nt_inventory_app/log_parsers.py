...
def _classify_type(descr: str, pid: str, name: str = '') -> str:
    ...
    # 1. NAME starts with "chassis"
    if _CHASSIS_NAME_RE.match(name.strip()):
        return 'CHASSIS'

    # 2. CHASSIS keyword in DESCR (not PEM/FAN/PWR)
    if re.search(r'\bCHASSIS\b', combined) and not re.search(r'\b(PEM|FAN|POWER|PWR)\b', combined):
        return 'CHASSIS'

    # 3. PID-first rules (more specific than DESCR)
    # A99-RP-xxx = SUPERVISOR (ASR9900 Route Processor)
    if re.match(r'^A99-RP', pid_u):
        return 'SUPERVISOR'
    # A99-xxx-LC, ASR-9903-LC = MODULE (Line Card)
    if re.search(r'A99-.*-LC$|ASR-990[0-9]-LC', pid_u):
        return 'MODULE'
    # A99-xxx-SE/FC/SIP = MODULE (Fabric/Service card)
    ...
    # 5. Fallback: PID matches known chassis pattern
